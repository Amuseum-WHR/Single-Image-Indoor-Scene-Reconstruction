...
def TwoDBB(gpu, pretrained_model, image):
    ...
    # plt.savefig('demo/2Ddetection.png')
    
    n_objects = bbox.shape[0]
    bbox_xy = [[int(bbox[i][1]), int(bbox[i][0]), int(bbox[i][3]), int(bbox[i][2])]  for i in range(n_objects)]
    # bbox_xy = [[int(bbox[i][0]), int(bbox[i][1]), int(bbox[i][2]), int(bbox[i][3])]  for i in range(n_objects)]
    dic = []
...
def main():
    ...
    parser.add_argument('--pretrained-model', default='./detection-pretrain/sunrgbd_model_65000.npz')
    parser.add_argument('--image', default='./img.jpg')
